...
test('redesigned operations shell keeps core accessibility and responsive affordances', () => {
  ...
  assert.match(css, /body \{[^}]*font-size: 13\.5px/);
  ...
  assert.match(app, /function updateCapabilityNavigation\(\)/);
  assert.match(html, /data-page="processes" data-capability="processes"/);
  assert.match(html, /data-page="watchdog" data-capability="watchdog"/);
  assert.match(app, /function formatHostCpuModel\(cpu = \{\}\)/);
  assert.match(app, /replace\(\/\\bCPU\\b\/gi, ''\)/);
  assert.match(app, /home assistant host\|x64\|x86_64\|arm64\|aarch64/);
  assert.match(app, /data-role="cpu-model"/);
  assert.match(css, /\.host-metric small\.cpu-model/);
  assert.doesNotMatch(app, /fleet\.processor\)\}: \$\{cpuModel/);
  ...
  assert.match(app, /host-meta[\s\S]*host-runtime[\s\S]*host-metrics/);
