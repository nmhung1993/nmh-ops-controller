...
function envelope(type, payload = {}) {
  state.sequence += 1;
  return { type, messageId: `linux-${crypto.randomUUID()}`, agentId: state.agentId || null, sentAt: new Date().toISOString(), seq: state.sequence, payload };
}
...
function scheduleReconnect() {
  ...
  console.log(`Linux Agent reconnect scheduled in ${delay}ms`);
  reconnectTimer = setTimeout(() => {
    reconnectTimer = null; reconnectTimerAt = 0;
    try { connect(); }
    catch (error) { console.error('Linux Agent connection setup failed:', error.message); socket = null; scheduleReconnect(); }
  }, delay);
}
function failConnection(current, reason) {
  ...
  console.error(`Linux Agent connection failed: ${reason}`);
  try { current.terminate(); } catch { }
  scheduleReconnect();
}
function connect() {
  if (socket?.readyState === WebSocket.OPEN || socket?.readyState === WebSocket.CONNECTING) return;
  console.log(`Linux Agent connecting to ${config.serverUrl}`);
  ...
  current.on('open', () => {
    ...
    console.log('Linux Agent WebSocket opened; sending hello');
    send(envelope('agent.hello', {
      installId: state.installId, token: state.token, hostname: os.hostname(), fingerprint: fingerprint(),
      platform: `Linux / ${os.type()} ${os.release()} ${os.arch()}`, version: VERSION, capabilities: getCapabilities()
    }));
  });
  current.on('message', raw => {
    ...
    if (message.type === 'server.pending') {
      ...
      console.log(`Linux Agent pending approval: ${state.agentId}`);
      ...
      console.log(`Linux Agent approved: ${state.agentId}`);
      ...
      console.log(`[Linux Agent Event] Server dispatched: ${eventName} (${issueId || 'N/A'})`);
  ...
  current.on('error', error => {
    console.error('Linux Agent connection error:', error.message);
  ...
  current.on('close', (code, reason) => {
    ...
    console.log(`Linux Agent disconnected (${code}): ${reason.toString()}`);
    scheduleReconnect();
  });
}
...
async function disks() {
  try {
    ...
    return output.split('\n').slice(1).map(line => line.trim().split(/\s+/)).filter(parts => parts.length >= 6 && parts[5].startsWith('/')).map(parts => ({ drive: parts[5], total: Number(parts[1]) * 1024, used: Number(parts[2]) * 1024, free: Number(parts[3]) * 1024 }));
  } catch { return []; }
}
...
function initialize() {
  ...
  console.log(`NMH Ops Controller Linux Agent ${VERSION} starting; server=${config.serverUrl}`);
